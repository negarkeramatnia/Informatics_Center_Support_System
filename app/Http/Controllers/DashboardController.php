...
class DashboardController extends Controller
{
    public function index()
    {
        ...
        $user = Auth::user();
        $viewData = []; // This array will hold all the data for our view.
        ...
        elseif ($user->role === 'support') {
            $viewData['supportData'] = [
                ...
                'assigned_to_me_tickets' => Ticket::where('assigned_to', $user->id)->whereIn('status', ['pending', 'in_progress'])->count(),
                'resolved_today_count' => Ticket::where('status', 'completed')->whereDate('updated_at', today())->count(),
                'new_high_priority_tickets' => Ticket::with('user:id,name')
        ...
        elseif ($user->role === 'admin') {
            $viewData['adminData'] = [
                ...
                'recent_system_tickets' => Ticket::with('user:id,name')
                                            ->latest()
                                            ->take(5)
                                            ->get(),
            ];
